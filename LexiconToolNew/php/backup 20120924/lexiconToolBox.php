...
function printClickableTokens($iWordFormId, $sAnalysesInDb, $iRowNr) {
  if(strlen($sAnalysesInDb) ) {
	...
	"else tokenAttestSelected($iWordFormId, $aTuple[1], $iRowNr);\">" .
	"$sPrintTuple</span>";
      $sSeparator = ' | ';
    }
  }
...
function printMainPage($sDatabase, $iId, $sName, $sMode, $iUserId, $sUserName,
  ...
  print "<td><input type=text value='$sPrintFilter' length=32 " .
    "onFocus=\"javascript: " .
  ...
  print "<td><input id=lemmaFilterInput type=text value='$sPrintLemmaFilter' " .
    "length=32 " .
    // onFocus
    "onFocus=\"javascript: " .
    // Next line is necessary because the very first time you click in these
...
function fillSentences($sDatabase, $iId, $sMode, $sWordForm, $iWordFormId,
  ...
  if( $oResult ) {
	...
	print $sDocumentHeader;
      ...
      fillSentencesForDoc($aRow['document_id'], $aRow['title'],
...
function fillSentencesForDoc($iDocumentId, $sDocumentTitle,
