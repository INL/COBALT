...
sub addToken {
  ...
  if( $hrState->{sState} eq 'inTagToBeIgnored') {
    ...
    my $arNewToken = ($arCoordinates) ?
      [$iOnset, $iOffset, cleanseWord($1), $1, @$arCoordinates]
	: [$iOnset, $iOffset, cleanseWord($1), $1];
    ...
    my $arNewToken = ($arCoordinates) ?
      [$iOnset, $iOffset, cleanseWord($sWord), $sWord, @$arCoordinates]
	: [$iOnset, $iOffset, cleanseWord($sWord), $sWord];
    push(@$arTokens, $arNewToken);
  }
}
...
sub cleanseWord_weg {
  ...
  if( $self->{bMapToLowercase} ) {
    return lc decode_entities ($sWord);
  }
  else {
    return decode_entities ($sWord);
  }
