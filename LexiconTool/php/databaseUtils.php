...
function getRow($oResult, $iOnset, &$oLastRow) {
  if(! $oLastRow )
    $oLastRow = mysql_fetch_assoc($oResult);
  
  while( $oLastRow && ($oLastRow['start_pos'] < $iOnset) )
    $oLastRow = mysql_fetch_assoc($oResult);

  if( $oLastRow && ($oLastRow['start_pos'] == $iOnset) ) {
    $oReturnRow = $oLastRow;
    $oLastRow = 0;
    return $oReturnRow;
  }
...
function getLateralTokenAttestations($iDocumentId, $sWordForm, $iStartPos) {
  printLog("Doing getLateralTokenAttestations($iDocumentId, $sWordForm, $iStartPos)"
	   . "\n");
  ...
  $sGetWordFormIdQuery = "SELECT wordform_id ".
  "FROM wordforms ".
  "WHERE wordform = \"$sWordForm\";";
  
  $oWordFormIdResult = doSelectQuery($sGetWordFormIdQuery);
  $iWordFormId = null;  
  if( ($aRow = mysql_fetch_assoc($oWordFormIdResult)) )
      $iWordFormId = $aRow['wordform_id'];
    mysql_free_result($oWordFormIdResult);
	if ( !$iWordFormId ) return false;
...
function addTypeFrequencies_tokens($sDatabase, $iDocumentId, $aTypes,
  ...
  $sTokenInsertQuery = "INSERT INTO " . getTheRightTokenDatabase() . ".tokens " .
...
// This function returns the true token database name,
// that is: if some specific token database was declared for a specific lexicon project (in array $asTokenDbName in globals.php), 
// this token database name should overrule the default token database (which is stated in $sTokenDbName).
function getTheRightTokenDatabase() {
	
	// default token database
	$rightTokenDbName = $GLOBALS['sTokenDbName'];
	
	// do we have an array with specific token database names for specific lexica?
	$tmpTokenDbArray = $GLOBALS['asTokenDbName'];
	
	// if so, check what's indide
	if ( isset($tmpTokenDbArray) )
		{
		// first: what is the current lexicon database name
		$tmpDatabase = isset( $GLOBALS['sDatabase'] ) ? $GLOBALS['sDatabase'] : $_REQUEST['sDatabase'];
		
		// second: is the current lexicon a key of the array?
		// if so, get its value: that's the token database that should be used for that lexicon project
		if ( isset($tmpTokenDbArray[ $tmpDatabase ]) )
			$rightTokenDbName = $tmpTokenDbArray[ $tmpDatabase ];
		}
	return $rightTokenDbName;
}
...
function insertTokens($sInsertQuery) {
  printLog("Doing (" . getTheRightTokenDatabase() . ") $sInsertQuery<br>\n");
  mysql_query($sInsertQuery);
  printMySQLError($sInsertQuery);
}
...
function getPageRank($sWordformLookedFor,
				$iId, $sMode, $sSortBy, $sSortMode, 
	...
	$sSetQuery = "SET @rank=0; ";
	doNonSelectQuery($sSetQuery);
	
	$sSelectQuery = "SELECT rank FROM (";
	$sSelectQuery .= "SELECT @rank:=@rank+1 AS rank, wordform_lowercase FROM (";
	$sSelectQuery .= "SELECT wordform, wordform_lowercase ";
  ...
  $sSelectQuery .= "ORDER BY $sSortByClause $sSortMode$sExtraSort " ;
	...
	if( ($oResult = doSelectQuery($sSelectQuery)) ) {
		if( ($aRow = mysql_fetch_assoc($oResult)) ) {
			$iWordformNr = intval($aRow['rank']);
			$iPageRank = floor( $iWordformNr / intval($iNrOfWordFormsPerPage) ) * intval($iNrOfWordFormsPerPage);
		  }
		mysql_free_result($oResult);
  }
...
// this function is called to update the attestations of the rows that were clicked on, and only those rows
function updateAnalysesOfSelectedRows($sDatabase, $iWordFormId, $sRowsData) {

	chooseDb($sDatabase);
	
	$aDocumentIdsAndStartPos = array();
	
	// $sRowsData consists of a set of data for each selected row:
	// iDocumentId, iSentenceNr, iWordFormId, iStartPos, iEndPos 
	// each element is comma separated, and each group of elements if pipe separated
	$aRowsDataForString = explode("|", $sRowsData);
	$aRowsData = array();	
	
	$aSentenceNrsToProcess = array();	
	
	for($i = 0; $i < count($aRowsDataForString); $i++) {
		
		$aParts = explode(",", $aRowsDataForString[$i]);
		$aRowsData[$i][0] = $aParts[0]; // iDocumentId
		$aRowsData[$i][1] = $aParts[1]; // iSentenceNr
		$aRowsData[$i][2] = $aParts[2]; // iWordFormId
		$aRowsData[$i][3] = $aParts[3]; // iStartPos
		$aRowsData[$i][4] = $aParts[4]; // iEndPos
		
		// gather the sentence numbers to process
		array_push($aSentenceNrsToProcess, $aParts[1]);
	}
	
	// this will give: analyzed_wordform_id, derivation_id, start_pos, analysesForSentence
	$oResults = getTokenAttestationsNarrowSet($aRowsData, $iWordFormId);
	
	// now build the html needed for each new row
	$aNewHtml = array();
	if($oResults) {
		
		// at the client side, we will read the sentence nr, and than get the corresponding html from the part after the 'sKetToValueSeparator'
		$sKeyToValueSeparator = ":::::"; 
		$oLastRow = 0;
		$aRows = false;
		
		while( ($aRow = mysql_fetch_assoc($oResults)) ) {
		  
			$iSentenceNr = $aRow['sentence_nr'];
			$iStartPos = $aRow['start_pos'];
			$iEndPos = $aRow['end_pos'];
			$sNewHtmlPart = clickableTokenAtt($iSentenceNr,
				    $aRow['analyzed_wordform_id'],
				    $aRow['analysesForSentence']);
			
			// the html for this row starts with a sentence number,
			// and if this is a new round for the same row, we just append the new html to the one that was generated at a previous round
			$sPrefix = ( isset($aNewHtml[$iSentenceNr]) ) ? $aNewHtml[$iSentenceNr]."<br>" : $iSentenceNr . $sKeyToValueSeparator;			
			$aNewHtml[$iSentenceNr] = $sPrefix . $sNewHtmlPart; 
			
			// remove the current sentence nr from the list of sentence numbers yet to process
			$aSentenceNrsToProcess = array_diff($aSentenceNrsToProcess, array($iSentenceNr));
				
		}
		
		// if some sentences returned no results, we need to build empty html for these empty attestations as well
		// (otherwise, a removed attestation won't be removed from the screen)
		foreach ($aSentenceNrsToProcess as $iSentenceNr)
		{
			$aNewHtml[$iSentenceNr] = $iSentenceNr . $sKeyToValueSeparator . "&nbsp;";
		}
		
	}
	
	// gather all the html output to send to the client now
	$sNewHtml = "";
	$sLineSeparator = "";
	foreach($aNewHtml as $iSentenceNr => $value) {
		$sNewHtml .= $sLineSeparator . $aNewHtml[$iSentenceNr];
		$sLineSeparator = "\n";
	}
...
function getTokenAttestationsNarrowSet($aRowsData, $iWordFormId) {
  printLog("Doing getTokenAttestationsNarrowSet(..., $iWordFormId)"
	   . "\n");
  // NOTE that the single and multiple lemmata analyses are mutually exclusive.
  // Whichever one is there ends up in the analysesForSentence column
  ...
  for($i = 0; $i < count($aRowsData); $i++) {
	  
	  // iDocumentId, iSentenceNr, iWordFormId, iStartPos, iEndPos 
	  $iDocumentId = $aRowsData[$i][0];
	  $iSentenceNr = $aRowsData[$i][1];  
	  $iStartPos   = $aRowsData[$i][3];
	  $iEndPos     = $aRowsData[$i][4];
	  
	  if ($sSelectQuery != '') $sSelectQuery .= " UNION ";

	  $sSelectQuery .= "(SELECT $iSentenceNr AS sentence_nr, ".
		"token_attestations.analyzed_wordform_id," .
		" token_attestations.derivation_id, start_pos, end_pos, " .
		" CONCAT(REPLACE(REPLACE(REPLACE(lemmata.modern_lemma, '>', '&gt;'), '<', '&lt;'), ' ', '&nbsp;')," .
		" IF(myPatterns.normalized_wordform IS NULL, ''," .
		" CONCAT(',&nbsp;',myPatterns.normalized_wordform))," .
		" IF(myPatterns.patterns IS NULL, '', CONCAT(',&nbsp;'," .
	...
	$sSelectQuery .= " ORDER BY start_pos";
...
function getWordformIdsForGroups($iUserId, $aSelected, &$aVerificationValues){
  $sSelectQuery =
    ...
    "  FROM wordform_groups, " . getTheRightTokenDatabase() . ".tokens" .
...
// function for sending a query and get all results (one field) in a comma separated list
function sendQueryAndGetResultsInCommaSeparatedList($sSelectQuery, $sResultFieldname){
	$sValues = '';  
	$cSeparator = '';
	
	if( ($oResult = doSelectQuery($sSelectQuery) ) ) {
    while( ($aRow = mysql_fetch_assoc($oResult)) ) {
      $sValues .= $cSeparator . $aRow[$sResultFieldname];
      $cSeparator = ', ';
    }
    mysql_free_result($oResult);
  }
...
function getMultipleLemmaSuggestionsQuery($sHeadword, $sModernWordForm,
  ...
  $sSelectQuery = "SELECT " .
    ...
    "  AND modern_lemma ";
  // Narrow it down somewhat for only the relevant lemma
...
function printLemmaSuggestions($sMenuMode, $sFirstPartOfMultiple,
  ...
  if( strlen($aLemmaArr[0]) ) {
	...
	$sTitleForPosInput =
	  str_replace('\\"', '&#34', "$sFirstPartOfMultiple$sLemmaSuggestion");
	...
	if( $sMenuMode == 'tokenAttSuggestion')
	  print " updateTokenAttInput(" .
	    "'$sTitle');\" ";
	else // NOTE that here we use a title ESCAPED DIFFERENTLY
	  if($sMenuMode == 'lemmaFilter')
	    print
	      " applyLemmaFilter('$sTitleForPosInput');\" ";
...
function newCorpus($sDatabase, $sNewCorpusName) {
  ...
  if (fullDatabaseMode($sDatabase)){
	...
	// create a table to keep copies of the original files
		$sCreateOriginalDocumentsTable = "CREATE TABLE IF NOT EXISTS original_documents (".
		"original_filename VARCHAR(255),".
		"original_filecontent MEDIUMTEXT,".
		"original_tokenized_filename VARCHAR(255),".
		"original_tokenized_filecontent MEDIUMTEXT".
		");";
		doNonSelectQuery($sCreateOriginalDocumentsTable);
  
	
	// now check if the 'doc_content' field already exists in the old 'documents' table
    $sDoesColumnExistQuery = "SELECT * FROM INFORMATION_SCHEMA.COLUMNS ".
		"WHERE TABLE_SCHEMA = '$sDatabase' AND TABLE_NAME = 'documents' AND COLUMN_NAME = 'doc_content'";
		
	// if the column doesn't exist, that means we must upgrade the database structure now
	$oResult = doSelectQuery($sDoesColumnExistQuery);
	$rows = mysql_num_rows($oResult);
	
	if ($rows==0){		
		// insert a new column into the documents table to enable file storage there (editable files)
		$sAddDocContentColumnQuery = "ALTER TABLE documents ADD doc_content MEDIUMTEXT after title;";
		doNonSelectQuery($sAddDocContentColumnQuery);
	}
  }  
...
function removeCorpus($sDatabase, $iCorpusId) {
...
function showCorpusFiles($sDatabase, $iUserId, $sUserName, $iCorpusId) {
  ...
  print "<div class=corpusFile>" .
    ...
    " onClick=\"javascript: showProgress(false, 'Uploading...<br> Please wait, this might take several minutes');\">" .
    "</form></span>" .
    "</td></tr></table></div>";
...
function addFileToCorpus($sDatabase, $iCorpusAddedTo, $sNewFile, $sAuthor) {
  ...
  if (fullDatabaseMode($sDatabase)){
	  ...
	  $sOriginalFile = str_replace( "_tokenized.tab", "", $sNewFile );
	  $sOriginalDocContent = readFileContent($sOriginalFile) ;

	  // insert the tokenized file content 
	  $sInsertQuery1 = "INSERT INTO documents (title, author, doc_content) " .
		"VALUES ('$sNewFile', '" . addslashes($sAuthor) . "', '". addslashes($sDocContent) ."')" .
		"ON DUPLICATE KEY UPDATE author = '" . addslashes($sAuthor) . "', doc_content = '". addslashes($sDocContent) ."';";  
		doNonSelectQuery($sInsertQuery1);
		
	  // insert the original uploaded file content
	  $sInsertQuery2 = "INSERT INTO original_documents ".
		"(original_filename, original_filecontent, original_tokenized_filename, original_tokenized_filecontent) " .
		"VALUES ('$sOriginalFile', '" . addslashes($sOriginalDocContent) . "', '$sNewFile', '". addslashes($sDocContent) ."')" .
		"ON DUPLICATE KEY UPDATE ".
		"original_filename = '" . $sOriginalFile . "', ".
		"original_filecontent = '". addslashes($sOriginalDocContent) . "', ".
		"original_tokenized_filename = '" . $sNewFile . "', ".
		"original_tokenized_filecontent = '". addslashes($sDocContent) .
		"';";
		
		doNonSelectQuery($sInsertQuery2);
...
// register the document in the token database
function insertDocumentInTokenDb($sFilePath, $iCorpusId) {
  ...
  $sSelectQuery = "SELECT document_path_id" .
    " FROM " . getTheRightTokenDatabase(). ".document_paths ".
  ...
  if( $iDocumentPathId == -1) {
    $sInsertQuery = "INSERT INTO " .
      getTheRightTokenDatabase() . ".document_paths (path) VALUES " .
      "('$sFilePath')";
    doNonSelectQuery($sInsertQuery);
    // Try again
    if( ($oResult = doSelectQuery($sSelectQuery)) ) {
  ...
  $sInsertQuery =
    "INSERT INTO " . getTheRightTokenDatabase() . ".document_indices " .
...
function removeFileFromCorpus($sDatabase, $iCorpusId, $iDocumentId) {
  // First delete from token database (has te be done while the records are
  // still in the documents table)
  ...
  deleteFilesFromDb($sDatabase, $iDocumentId);
}
...
function unlinkFile($sDocumentPath) {
  ...
  // of course, this is only relevant if NOT in full database mode
  if ( !fullDatabaseMode($sDocumentPath) ){
...
function deleteFilesFromDb($sDatabase, $sDocumentIds) {
  ...
  if (fullDatabaseMode($sDatabase)){
	...
	$sDeleteQuery = "DELETE FROM original_documents WHERE original_tokenized_filename IN ".
	"(SELECT title FROM documents WHERE document_id IN ($sDocumentIds))";
	doNonSelectQuery($sDeleteQuery);
	}
...
function getDocumentContentFromDb($sDocumentTitle){

	$sSelectQuery = "SELECT doc_content FROM documents ".
		"WHERE title = '$sDocumentTitle'";
	
	$sDocContent = "";
	if( ($oResult = doSelectQuery($sSelectQuery)) ) {
    if( ($aRow = mysql_fetch_assoc($oResult)) )
      $sDocContent .= $aRow['doc_content'];
    mysql_free_result($oResult);
	}
	return $sDocContent;
}

// update the content of a document, given its title (=absolute path)
function updateDocumentContentInDb($sDocumentTitle, $sFileContent){

	$sUpdateQuery = "UPDATE documents " .
		"SET doc_content = '" . addslashes($sFileContent) . "' " .
		"WHERE title = '$sDocumentTitle'";
...
function deleteFilesFromTokenDb($sDocumentIds, $iCorpusId) {
  ...
  $sSelectQuery =
    ...
    "  FROM " . getTheRightTokenDatabase() . ".document_paths, " .
    "       " . getTheRightTokenDatabase() . ".document_indices, documents " .
  ...
  if( strlen($sDocumentPathIdsToBeDeleted) ) {
    $sDeleteQuery =
      "DELETE FROM " . getTheRightTokenDatabase().".document_paths " .
  ...
  if( strlen($sDocumentPathIds) ) {
    $sDeleteQuery =
      "DELETE FROM " . getTheRightTokenDatabase() . ".document_indices ".
      "WHERE corpus_id = $iCorpusId" .
      "  AND lexicon_database_id = " . $GLOBALS['iTokenDbDatabaseId'] .
...
function getGroupMemberOnsetsPerDocId($sDocIds, $iWordFormId) {
  $sSelectQuery = "SELECT document_id, GROUP_CONCAT(onset) tokenOnsets" .
    "  FROM " . getTheRightTokenDatabase() . ".tokens" .
...
function addToGroup($iUserId, $iDocumentId, $iHeadWordOnset, $iHeadWordOffset,
  ...
  $sSelectQuery =
    "SELECT wordform_id FROM " . getTheRightTokenDatabase() . ".tokens " .
    "WHERE document_id = $iDocumentId" .
    "  AND lexicon_database_id = " . $GLOBALS['iTokenDbDatabaseId'] .
    "  AND onset = $iOnset";
...
function cwUpdateTokens_tokenAttestations($aChanges, $aNewOnsetOffsets,
					  $iOffsetChange) {
  $sDeleteClauses = $sTokenAttUpdateQuery_startPos =
    $sTokenAttUpdateQuery_endPos = $sTokenUpdateQuery_onset =
    $sTokenUpdateQuery_offset = $sOr = '';
  $aTokenAttUpdateQueries_startPos = array();
  $aTokenAttUpdateQueries_endPos = array();
  $aTokenUpdateQueries_onset = array();
  ...
  foreach($aChanges as $iDocId => $aChange) {
    ...
    $iPreviousOffsetChange = 0;
    for($i = 0; $i < $iAmountOfChanges; $i++ ) {
      $sTokenAttUpdateQuery_startPos =
	"SET start_pos = start_pos + " . $aChange[0][1] .
	...
	"$sOr(document_id = $iDocId AND onset = " . $aChange[$i][0] . ")";
      $sOr = " OR ";
      $iPreviousOffsetChange = $aChange[$i][1];
    }
  }
  ...
  for($i = 0; $i < count($aTokenUpdateQueries_onset); $i++) {
    // First the offset, then the onset
    doNonSelectQuery($aTokenUpdateQueries_offset[$i]);
    doNonSelectQuery($aTokenUpdateQueries_onset[$i]);
  }
  ...
  $sInsertQuery = "INSERT INTO " . getTheRightTokenDatabase() . ".tokens " .
  ...
  if( $iOffsetChange != 0) {
    for($i = 0; $i < count($aTokenAttUpdateQueries_startPos); $i++) {
      // First token_attestations end_pos
      doNonSelectQuery("UPDATE token_attestations " .
		       $aTokenAttUpdateQueries_endPos[$i]);
      // The token_attestations start_pos
      doNonSelectQuery("UPDATE token_attestations " .
		       $aTokenAttUpdateQueries_startPos[$i]);

      // Same for verifications
      doNonSelectQuery("UPDATE token_attestation_verifications " .
		       $aTokenAttUpdateQueries_endPos[$i]);
      // The token_attestations start_pos
      doNonSelectQuery("UPDATE token_attestation_verifications " .
		       $aTokenAttUpdateQueries_startPos[$i]);
...
function cwSingleWord($iUserId, $iOldWordFormId, $iNewWordFormId,
  ...
  // Then we update all the token attestations
  $sUpdateCondition = $sOr = '';
  foreach($aChanges as $iDocId => $aChange) {
    for($i=0; $i < count($aChange); $i++ ) {
      $sUpdateCondition .=
	"$sOr(document_id = $iDocId AND start_pos = " . $aChange[$i][0] . ")";
      $sOr = " OR ";
    }
  }

  foreach( $aAwfIdMapping as $iOldAwdId => $iNewAwfId ) {
    // Token attestations
    $sUpdateQuery =
      "UPDATE token_attestations" .
      "   SET analyzed_wordform_id = $iNewAwfId" .
      " WHERE analyzed_wordform_id = $iOldAwdId" .
      "   AND ($sUpdateCondition)";
    doNonSelectQuery($sUpdateQuery);
  }
  
  // Same idea for the token attestation verifications, but now change the
  // word form id's
  $sUpdateQuery =
    "UPDATE token_attestation_verifications" .
    "   SET wordform_id = $iNewWordFormId" .
...
function cwCopyAnalyzedWordForms($iUserId, $iOldWordFormId, $iNewWordFormId) {
  ...
  // Now for the mapping between the two
  $sSelectQuery =
    "SELECT a1.analyzed_wordform_id oldAwfId, a2.analyzed_wordform_id newAwfId".
    "  FROM analyzed_wordforms a1, analyzed_wordforms a2" .
    " WHERE a1.wordform_id = $iOldWordFormId" .
    "   AND a2.wordform_id = $iNewWordFormId" .
    "   AND a1.part_of_speech = a2.part_of_speech" .
    "   AND a1.lemma_id = a2.lemma_id" .
    "   AND a1.multiple_lemmata_analysis_id = a2.multiple_lemmata_analysis_id" .
    "   AND a1.derivation_id = a2.derivation_id";

  $aAwfIdMapping = array();
  if( ($oResult = doSelectQuery($sSelectQuery)) ) {
    while( ($aRow = mysql_fetch_assoc($oResult)) ) {
      $aAwfIdMapping[$aRow['oldAwfId']] = $aRow['newAwfId'];
    }
    mysql_free_result($oResult);
  }
  return $aAwfIdMapping;
}
...
function cwUpdateTypeFrequencies_dontShows($iOldWordFormId, $aNewWordFormIds,
					   $aWordFormFrequencies, $aChanges) {
  $sStillExistsClause = $sOr = $sComma = $sInsertValues = '';
  $aTypeFrequencyUpdateQueries = array();
  $bKeepWordform = FALSE;
  foreach($aChanges as $iDocId => $aChange) {
    $iFreq = count($aChange);
	...
	array_push($aTypeFrequencyUpdateQueries,
		   "UPDATE type_frequencies" .
		   "   SET frequency = frequency + $iNewFreq" .
		   " WHERE document_id = $iDocId" .
		   "   AND wordform_id = $iNewWordFormId");
      }
    }
    ...
    if( ! $bKeep )
      array_push($aTypeFrequencyUpdateQueries,
		 "UPDATE type_frequencies" .
		 "   SET frequency = frequency - $iFreq" .
		 " WHERE document_id = $iDocId" .
		 "   AND wordform_id = $iOldWordFormId");
  ...
  if( count($aTypeFrequencyUpdateQueries) ) {
    foreach( $aTypeFrequencyUpdateQueries as $sTypeFrequencyUpdateQuery) {
      doNonSelectQuery($sTypeFrequencyUpdateQuery);
    }
  }
...
function cwUpdateWordFormGroups($aChanges) {
  ...
  // NOTE that we don't use CASE statements anymore because they are SLOW.
  // We use multiple updates of the same tables which is quicker (on large
  // tables).
  // We only use $aChanges[0][1].
  foreach($aChanges as $iDocId => $aChange) {
    $iAmountOfChanges = count($aChanges[$iDocId]);
    printLog("change length: $iAmountOfChanges\n");
    $iPreviousOffsetChange = 0;
    for($i = 0; $i < $iAmountOfChanges; $i++ ) {
      $sUpdateQuery_onset = "UPDATE wordform_groups SET onset = onset + " .
  ...
  for($i = 0; $i < count($aUpdateQueries_onset); $i++) {
    // First the offsets then the onsets
    doNonSelectQuery($aUpdateQueries_offset[$i]);
    doNonSelectQuery($aUpdateQueries_onset[$i]);
  }
}
...
function getUnlemmatizedWordforms($sDatabase){
	...
	$sQuery = "SELECT DISTINCT tf.wordform_id, wf.wordform ".
		"FROM wordforms wf ".
		"RIGHT JOIN ".
		"type_frequencies tf ".
		"on wf.wordform_id = tf.wordform_id ".
		"LEFT JOIN ".
	...
	print "List of wordforms that were not yet lemmatized in the current project:<br><br>";
		
	while( ($aRow = mysql_fetch_assoc($oResult)) ) {
			print "- <span id='".$aRow['wordform_id'] ."' title='wordform_id = ".$aRow['wordform_id'] ."'>". $aRow['wordform']."</span><br>";
	}		
	mysql_free_result($oResult);
}

// get list of wordforms which are only partially lemmatized (some occurences are, other are not)
function getPartiallyUnlemmatizedWordforms($sDatabase){

	chooseDb($sDatabase);

	$sQuery = "SELECT tfw.wordform_id, tfw.wordform ".
		" FROM ".
		" (SELECT wordform_id, COUNT(*) AS frequency ".
		"   FROM token_attestation_verifications ".
		"   GROUP BY wordform_id ".
		"  ) tav ".
		" RIGHT JOIN ".
		" (SELECT tf.wordform_id, SUM(tf.frequency) AS frequency, wf.wordform ".
	...
	print "List of wordforms that were but partly lemmatized in the current project:<br><br>";
		
	while( ($aRow = mysql_fetch_assoc($oResult)) ) {
	
			$sWordformId = $aRow['wordform_id'];
			$sWordform   = $aRow['wordform'];			
			print "- <span id='".$sWordformId ."' title='wordform_id = ".$sWordformId ."'>". $sWordform."</span><br>";						
	}		
...
function doSelectQuery($sSelectQuery) {
  ...
  mysql_query("set session group_concat_max_len = 300000;", $GLOBALS['dbh']);
