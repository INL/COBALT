...
function toggleTokenAttestation(oTokenVerificationBox, iDocumentId,
  ...
  // See out if we are to check or uncheck it, based on the row that was clicked
  ...
  if( sSelecteds.length ) {
    ...
    xmlHttp.onreadystatechange=function() {
      ...
      if(xmlHttp.readyState == 4) {
		...
		if (bAddCurrentRow && aSelected.length == 1) {
  ...
  {
	// if nothing happens (no selection) make at least the progress bar invisible again
		oProgressBar.style.visibility = 'hidden';
		document.body.style.cursor = "default";
  }
}
